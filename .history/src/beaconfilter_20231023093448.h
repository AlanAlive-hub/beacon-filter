#ifndef BEACON_FILTER_H_
#define BEACON_FILTER_H_
...
class BeaconFilter {
  ...
  inline void GenerateIndexTagHash(const ItemType& item, size_t* index,
    ...
    //HashUtil::BobHash((const void*) (item), 4) & table_->INDEXMASK;
 ...
 public:
  ...
  // Extra the flag from each tag
  void BuildFlagMat(int zip=0) const { table_->BuildFlagMat(zip); }
  ...
  inline double lf() const {return LoadFactor();}
...
Status BeaconFilter<ItemType, bits_per_item, TableType, HashFamily>::AddImpl(
  ...
  for (uint32_t count = 0; count < kMaxCuckooCount; count++) {
    ...
    oldtag = 0;
    oldflag = 0;
    if (table_->InsertTagToBucket(curindex, curtag, curflag, kickout, oldtag, oldflag)) {
      num_items_++;
      return Ok;
    }
    if (kickout) {
...
Status BeaconFilter<ItemType, bits_per_item, TableType, HashFamily>::Delete(
  ...
  if (table_->DeleteTagFromBucket(i1, tag, 0)) {
    num_items_--;
    goto TryEliminateVictim;
  } else if (table_->DeleteTagFromBucket(i2, tag, 1)) {
    num_items_--;
    goto TryEliminateVictim;
...
std::string BeaconFilter<ItemType, bits_per_item, TableType, HashFamily>::Info() const {
  ...
  ss << "BeaconFilter Status:\n"
     ...
     << "\t\tHashtable size: " << ((table_->SizeInBytes()+(Size()/8)) >> 10) << " KB\n";
  if (Size() > 0) {
    ss << "\t\tbit/key:   " << BitsPerItem()+1 << "\n";
  } else {
    ss << "\t\tbit/key:   N/A\n";
  }
  return ss.str();
