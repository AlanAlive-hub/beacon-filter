#ifndef BEACON_FILTER_CUCKOO_FILTER_H_
#define BEACON_FILTER_CUCKOO_FILTER_H_
  ...
  inline bool FindTagInBuckets(const size_t i1, const size_t i2,
    ...
    for (size_t j = 0; j < kTagsPerBucket; j++) {
      if (ReadTag(i1, j) == tag && beacon[i1][kTagsPerBucket-j-1] == 0) 
        return true;
      if (ReadTag(i2, j) == tag && beacon[i2][kTagsPerBucket-j-1] == 1)
        return true;
    }
    return false;
  }
  ...
  inline bool InsertTagToBucket(const size_t i, const uint32_t tag, uint8_t flag, 
    ...
    for (size_t j = 0; j < kTagsPerBucket; j++) {
      if (ReadTag(i, j) == 0) {
        WriteTag(i, j, tag);
        if(kickout & flag) 
          beacon[i][kTagsPerBucket-j-1] = flag;  // 区分p1和p2
        return true;
      }
    }
    if (kickout) {
...
#endif  // BEACON_FILTER_CUCKOO_FILTER_H_
