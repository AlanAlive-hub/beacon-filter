...
int main(int argc, char **argv) {
  ...
  for (size_t i = 0; i < insert_items; i++, num_inserted++) {
    if (filter.Add(*(ipset_uniq.find(i))) != beaconfilter::Ok) {
      break;
    }
  }
  ...
  for (size_t i = 0; i < num_inserted; i++) {
    assert(filter.Contain(*(ipset_uniq.find(i))) == beaconfilter::Ok);
  }
  ...
  for (size_t i = insert_items; i < ipset_uniq.size(); i++) {
    if (filter.Contain(*(ipset_uniq.find(i))) == beaconfilter::Ok) {
      false_queries++;
    }
    total_queries++;
  }
  ...
  for (size_t i = 0; i < num_inserted; i++)
  {
    auto item = *(ipset_uniq.find(i));
    assert(filter.Contain(item) == beaconfilter::Ok);
    filter.Delete(item);
  }
  return 0;
}
